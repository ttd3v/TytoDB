...
use crate::{Token, alba_types::AlbaTypes, gerr, query::PrimitiveQueryConditions, row::Row};
...
impl QueryConditions {
    pub fn from_primitive_conditions(
        primitive_conditions: PrimitiveQueryConditions,
        column_properties: &HashMap<String, AlbaTypes>,
        primary_key: String,
    ) -> Result<Self, Error> {
        ...
        for i in condition_logical_gates_vec {
            condition_logical_gates.insert(
                i.0,
                match i.1 {
                    'a' | 'A' => LogicalGate::And,
                    'o' | 'O' => LogicalGate::Or,
        ...
        for (index, value) in condition_chunk.iter().enumerate() {
            ...
            let column = if let Token::String(name) = value.0 {
                ...
                return Err(gerr(
                    "Failed to get QueryConditions, but failed to gather the column_name.",
                ));
            };
            ...
            let operator = if let Token::Operator(operator_name) = value.1 {
                match operator_name.as_str() {
                    ...
                    _ => return Err(gerr("Failed to get operator, invalid token contant.")),
                }
            } else {
            ...
            let column_value = if let Some(column_type) = column_properties.get(&column) {
                match column_type {
                    ...
                    AlbaTypes::HugeInt(_) => match value.2 {
                    ...
                    AlbaTypes::LightPassword(_) => {
                        if let Token::Bytes(mut b) = value.2 {
                            ...
                            return Err(gerr(
                                "No bytes found for LightPassword in the ComparisionToken",
                            ));
                        }
                    }
                    AlbaTypes::MediumPassword(_) => {
                        if let Token::Bytes(mut b) = value.2 {
                            ...
                            return Err(gerr(
                                "No bytes found for MediumPassword in the ComparisionToken",
                            ));
                        }
                    }
                    AlbaTypes::HeavyPassword(_) => {
                        if let Token::Bytes(mut b) = value.2 {
                            ...
                            return Err(gerr(
                                "No bytes found for HeavyPassword in the ComparisionToken",
                            ));
                        }
                    }
                    AlbaTypes::Slice4(_) => {
                    ...
                    AlbaTypes::NONE => {
                        return Err(gerr(
                            "Failed to extract the value from the column_properties",
                        ));
                    }
                }
            } else {
                return Err(gerr(
                    "Failed to generate QueryConditions, that happened because no column_property has been found with the given column-names",
                ));
            };

            let gate = condition_logical_gates.get(&index).map(|a| a.clone());
            ...
            let check = match query_condition.operator {
                ...
                | Operator::LowerEquality => {
                    ...
                    let result = match (row_value, value) {
                        (AlbaTypes::NanoInt(x), AlbaTypes::NanoInt(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                            }
                        }
                        (AlbaTypes::Short(x), AlbaTypes::Short(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                        ...
                        (AlbaTypes::HugeInt(x), AlbaTypes::HugeInt(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                        ...
                        (AlbaTypes::UNanoInt(x), AlbaTypes::UNanoInt(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                        ...
                        (AlbaTypes::UShort(x), AlbaTypes::UShort(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                        ...
                        (AlbaTypes::UInt(x), AlbaTypes::UInt(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                        ...
                        (AlbaTypes::UBigint(x), AlbaTypes::UBigint(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                        ...
                        (AlbaTypes::UHugeInt(x), AlbaTypes::UHugeInt(y)) => {
                            if lower {
                                if equality { *x <= *y } else { *x < *y }
                            } else {
                                if equality { *x >= *y } else { *x > *y }
                ...
                Operator::Different => *value != *row_value,
                Operator::StringContains | Operator::StringCaseInsensitiveContains => {
                    ...
                    let row_string = match row_value {
                        ...
                        _ => {
                            return Err(gerr(
                                "Invalid, the entered type cannot make string operations",
                            ));
                        }
                    };
                    ...
                    if case_insensitive {
                        row_string
                            .to_lowercase()
                            .contains(&value_string.to_lowercase())
                    } else {
    ...
    pub fn query_type(&self) -> Result<QueryType, Error> {
        ...
        let chain: Vec<(QueryConditionAtom, Option<LogicalGate>)> = self
            .chain
            .clone()
            .into_iter()
            .filter(|p| p.0.column == pk)
            .collect();
        if chain.is_empty() {
            return Ok(QueryType::Scan);
        }
        ...
        for i in chain {
            match i.0.operator {
                Operator::Equal | Operator::StrictEqual => index_array.push(get_index(i.0.value)),
                _ => {
                    continue;
                }
            }
