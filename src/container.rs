...
pub fn get_index(i: AlbaTypes) -> u64 {
    ...
    match i {
        ...
        AlbaTypes::NanoString(b)
        | AlbaTypes::SmallString(b)
        | AlbaTypes::MediumString(b)
        | AlbaTypes::BigString(b)
        | AlbaTypes::LargeString(b)
        | AlbaTypes::Text(b)
        | AlbaTypes::Email(b) => {
            b.hash(&mut hasher);
            hasher.finish()
    ...
    pub fn new(
        ...
        for index in 0..((columns.len() + column_names.len()) / 2) {
            let name = match column_names.get(index) {
                ...
                None => {
                    return Err(gerr(
                        "Failed to create container, the size of column types and names must be equal. And this error is a consequence of that property not being respected.",
                    ));
                }
            };
...
impl Container {
    pub fn build_btree(&mut self) -> Result<(), Error> {
        ...
        let total_rows =
            (self.file.metadata()?.len() as usize - headers_offset as usize) / element_size;
        ...
        for i in 0..count_its {
            ...
            self.file.read_exact_at(&mut buffer, file_offset).unwrap();
...
impl Container {
    pub fn get_next_addr(&self) -> Result<u64, Error> {
        let m = self.mvcc.0.iter().max_by(|a, b| a.0.cmp(&b.0));
    ...
    pub fn vacuum(&mut self) -> Result<(), Error> {
        ...
        let length = (self.file.metadata()?.size() - self.headers_offset) / element_size;
        ...
        for (dead, alive) in pairs {
            ...
            self.file.read_exact_at(&mut buffer, alive_offset)?;
            let row_pk = self.deserialize_row(&buffer)?[0].clone();
            let dead_offset = (dead * element_size) + self.headers_offset;
            self.file.write_all_at(&buffer, dead_offset)?;
            self.file
                .write_all_at(&vec![255u8; self.element_size], alive_offset)?;
            dead_v.push(Request {
                method: RequestMethods::Write,
        ...
        if rows_to_remove > 0 {
            let new_len = self
                ...
                .metadata()?
                .size()
                .saturating_sub(rows_to_remove * element_size)
                .max(self.headers_offset);
    ...
    pub fn push_row(&mut self, data: Vec<AlbaTypes>) -> Result<(), Error> {
        ...
        self.index_map.request(&mut re).unwrap();
    ...
    pub fn commit(&mut self) -> Result<(), Error> {
        let mut dataset_batch = Vec::with_capacity(self.mvcc.0.len());
        let mut index_batch = Vec::with_capacity(self.mvcc.0.len());
        let file = self.file.as_raw_fd();
        let empty = Arc::new(vec![255u8; self.element_size]);
        for i in self.mvcc.0.iter() {
    ...
    pub fn deserialize_row(&self, buf: &[u8]) -> Result<Vec<AlbaTypes>, Error> {
        ...
        for column_type in &self.columns() {
            ...
            let value = match column_type {
                ...
                AlbaTypes::Short(_) => {
                    let bytes: [u8; 2] = field_buf[0..2]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read short: {}", e)))?;
                ...
                AlbaTypes::UShort(_) => {
                    let bytes: [u8; 2] = field_buf[0..2]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read ushort: {}", e)))?;
                ...
                AlbaTypes::UInt(_) => {
                    let bytes: [u8; 4] = field_buf[0..4]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read uint: {}", e)))?;
                    AlbaTypes::UInt(u32::from_le_bytes(bytes))
                }

                AlbaTypes::Bigint(_) => {
                    let bytes: [u8; 8] = field_buf[0..8]
                        .try_into()
                ...
                AlbaTypes::UBigint(_) => {
                    let bytes: [u8; 8] = field_buf[0..8]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read ubigint: {}", e)))?;
                ...
                AlbaTypes::HugeInt(_) => {
                    let bytes: [u8; 16] = field_buf[0..16]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read hugeint: {}", e)))?;
                ...
                AlbaTypes::UHugeInt(_) => {
                    let bytes: [u8; 16] = field_buf[0..16]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read uhugeint: {}", e)))?;
                    AlbaTypes::UHugeInt(u128::from_le_bytes(bytes))
                }

                AlbaTypes::Geo(_) => {
                    let lat_bytes: [u8; 8] = field_buf[0..8]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read geo lat: {}", e)))?;
                    let lon_bytes: [u8; 8] = field_buf[8..16]
                        .try_into()
                        .map_err(|e| gerr(&format!("Failed to read geo lon: {}", e)))?;
                    AlbaTypes::Geo((f64::from_le_bytes(lat_bytes), f64::from_le_bytes(lon_bytes)))
                }

                AlbaTypes::Text(_) => {
                    let s = String::from_utf8_lossy(field_buf)
                        .trim_end_matches('\0')
                        .to_string();
                    AlbaTypes::Text(s)
                }
                ...
                | AlbaTypes::LargeString(_) => {
                    ...
                    let s = String::from_utf8(field_buf[8..data_end].to_vec())
                        .map_err(|e| gerr(&format!("Invalid string: {}", e)))?;
                    match column_type {
